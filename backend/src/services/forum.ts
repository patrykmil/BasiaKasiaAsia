...
export class ForumService {
   ...
   * @returns Promise resolving to array of Forums
   */
  public async getForumsByCategoryId(
    categoryId: number,
    limit: number = 50,
    offset: number = 0
  ): Promise<ForumResponse[]> {
    return this.getAllForums(limit, offset, categoryId);
  }
   ...
   */
  public async createForum(forumData: {
    title: string;
    description?: string;
    created_by: number;
  }): Promise<ForumResponse> {
    try {
      ...
      const createdForum = await this.getForumById(forum.forum_id, false);
  ...
  public async updateForum(
    ...
    try {
      ...
      return this.getForumById(forumId, false);
    } catch (error) {
      throw new Error(
   ...
   */
  public async deleteForum(forumId: number): Promise<boolean> {
    try {
