...
export class CommentService {
    ...
    try {
      ...
      const commentsWithReplies = await Promise.all(
        topLevelComments.map(async (comment) => {
          const replies = await this.getRepliesForComment(comment.comment_id);
   ...
   */
  public async getRepliesForComment(
    ...
    try {
      ...
      const repliesWithNested = await Promise.all(
        replies.map(async (reply) => {
          const nestedReplies = await this.getRepliesForComment(
            reply.comment_id,
            maxDepth - 1
          );
          const repliesCount = await Comment.count({
  ...
  public async getCommentById(
    ...
    try {
      ...
      if (includeReplies) {
        replies = await this.getRepliesForComment(commentId);
  ...
  public async createComment(commentData: {
    ...
    try {
      ...
      const createdComment = await this.getCommentById(comment.comment_id, false);
  ...
  public async updateComment(
    ...
    try {
      ...
      return this.getCommentById(commentId, false);
    } catch (error) {
      throw new Error(
   ...
   * @param parentCommentId parent comment ID
   */
  private async deleteCommentReplies(parentCommentId: number): Promise<void> {
    try {
      const replies = await Comment.findAll({
        where: { parent_comment_id: parentCommentId },
      ...
      for (const reply of replies) {
        await this.deleteCommentReplies(reply.comment_id);
      }
   ...
   */
  public async getCommentStats(threadId: number) {
    try {
