...
export class ThreadService {
   ...
   * @returns Promise resolving to array of Threads
   */
  public async getThreadsByForumId(
    forumId: number,
    limit: number = 20,
    offset: number = 0
  ): Promise<ThreadResponse[]> {
    return this.getAllThreads(limit, offset, forumId);
  }
   ...
   * @returns Promise resolving to array of Threads
   */
  public async getThreadsByUserId(
    userId: number,
    limit: number = 20,
    offset: number = 0
  ): Promise<ThreadResponse[]> {
   ...
   */
  public async createThread(threadData: {
    title: string;
    description?: string;
    forum_id: number;
    user_id: number;
    ...
    try {
      ...
      const createdThread = await this.getThreadById(thread.thread_id, false);
  ...
  public async updateThread(
    ...
    try {
      ...
      return this.getThreadById(threadId, false);
    } catch (error) {
      throw new Error(
   ...
   */
  public async deleteThread(threadId: number): Promise<boolean> {
    try {
