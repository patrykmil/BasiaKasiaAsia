  ...
  try {
    ...
    const users = [
      ...
      },
      {
        firstName: 'Piotr',
        lastName: 'Wiśniewski',
        username: 'p_wisniewski',
        email: 'piotr.w@domena.com',
        age: 42,
      ...
      },
      {
        ...
        lastName: 'Kamiński',
        username: 'mkaminski',
        email: 'michal.kaminski@fake.net',
        age: 38,
    ...
    await Comment.create({
      content:
        "Definitely add trotters! It's the collagen that makes the soup thick. I usually mix pork loin bones with trotters in a 2:1 ratio. And boil for a minimum of 12 hours, 8 is a bit too short for full extraction.",
    ...
    await Comment.create({
      content:
        'I had this problem too. It\'s also important to boil off the scum vigorously at the beginning and change the water, then maintain a "rolling boil" so the fat emulsifies with the water. Good luck!',
      thread_id: thread1.thread_id,
    ...
    await Comment.create({
      content:
        'Check online Asian food stores, sometimes they stock it, but the price is killer (approx. 300-400 PLN/kg). I recently saw it in premium delis in Warsaw, but it sells out fast.',
    ...
    await Comment.create({
      content:
        "If it's veneer, I beg you, don't use a belt sander! Only a random orbital sander or do it by hand. I would start gently with 180 grit. If the varnish is thick and hard (chemically hardened), it's better to use a paint stripper gel first, and only sand at the very end.",
    ...
    await Comment.create({
      content:
        'I agree with Paul. The veneer on furniture from that era can be very thin. Once you clean it, I recommend hard wax oil instead of varnish – it will bring out the grain beautifully.',
      thread_id: thread3.thread_id,
    ...
    await Comment.create({
      content:
        "For the start, take a 5mm braided cord (the one with a core). Twisted looks beautiful because you can comb out the ends, but it's harder to work with because it unravels while tying. Braided is more forgiving of mistakes.",
    ...
    await Comment.create({
      content:
        'Unfortunately, this is typical for variegatas. Those white parts have no chlorophyll and are very sensitive. It might have too little humidity in the air. Do you have a humidifier? It should be at min. 60%.',
    ...
    await Comment.create({
      content:
        "Check the roots too. If the browning starts from the edges, it's humidity, but if the spots are soft and mushy, it could be overwatering and fungus.",
      thread_id: thread5.thread_id,
      user_id: userMap['tommylew'],
    });
    ...
    await Comment.create({
      content:
        'I would add supplemental lighting. One meter from a window in winter is like a darkroom for it. The white parts die because the plant rejects "parasitic" tissue that doesn\'t produce energy when there isn\'t enough light.',
      thread_id: thread5.thread_id,
      user_id: userMap['aga_z'],
    });
    ...
    await Comment.create({
      content:
        "Confirming what Aga wrote. Buy a grow light bulb. And don't mist the leaves! That often causes fungal infections on the white parts.",
      thread_id: thread5.thread_id,
      user_id: userMap['pdabrowski'],
    });
